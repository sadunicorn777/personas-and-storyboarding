...
const updatePage = async () => {
  ...
  const fruitResponseContainer = document.createElement('div');
  fruitResponseContainer.className = "button-response-container";
  gallery.append(fruitResponseContainer);

  fruitsArray.map((fruit) => {

    if (fruit.nutritions.sugar > 15) {
      const message = "The " + fruit.name + " has " + fruit.nutritions.sugar + " grams of sugar which is a lot!!!";
      const newElement = document.createElement('p');
      newElement.className = "sugar-message";
      newElement.innerHTML = message;
      fruitResponseContainer.append(newElement);
    }
    console.log(fruit);
  })
